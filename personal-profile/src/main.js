    ...
    if (!ticking) {
        window.requestAnimationFrame(() => {
            if (window.scrollY > lastScrollY + 10) {
                navbar.style.top = '-70px'; // Ẩn khi cuộn xuống
...
// XÓA hoặc comment đoạn này nếu bạn không muốn xử lý AJAX
document.addEventListener('DOMContentLoaded', () => {
...
document.body.addEventListener('click', function(e) {
    // Áp dụng cho mọi ảnh trong .project-card hoặc .projects-list
    if (e.target.matches('.project-card img, .projects-list img')) {
